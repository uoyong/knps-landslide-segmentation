"""Submit exactly the reviewed 17th notebook; reuse the local participation key."""
import os,json,re
...
def tracked_submit(*args,**kwargs):
    ...
    (root/'17_submission/submission_receipt.json').write_text(json.dumps(response,ensure_ascii=False,indent=2))
    return response
client.submit=tracked_submit
os.chdir(root/'17_submission')
raise SystemExit(cli.main(['submit','--notebook','predict.ipynb','--model-name','sam3-spectral-refine-v17']))
